	...
	public class SqlScripts
	{
		...
		public static string GetUserScriptNames(SqlString nvchrDomain, IDbConnection connection, IDbTransaction transaction)
		{
			...
			try
			{
				...
				try
				{
					using(SqlDataAdapter dataAdapter = new SqlDataAdapter("usp_N_getUserScriptNames_XML", (SqlConnection)connection))
					{
						...
						if (Constants.TRACING_ENABLED)
						{
							MDMTraceHelper.EmitTraceEvent(System.Diagnostics.TraceEventType.Verbose, "Calling:[usp_N_getUserScriptNames_XML]");
						}
		...
		public static string GetUserScripts(SqlString nvchrName, SqlString nvchrDomain, IDbConnection connection, IDbTransaction transaction)
		{
			...
			try
			{
				...
				try
				{
					using(SqlDataAdapter dataAdapter = new SqlDataAdapter("usp_N_getUserScript_XML", (SqlConnection)connection))
					{
						SqlCommand command = dataAdapter.SelectCommand;
						command.Transaction = (SqlTransaction)transaction;
						command.CommandType = CommandType.StoredProcedure;
						...
						if (Constants.TRACING_ENABLED)
						{
							MDMTraceHelper.EmitTraceEvent(System.Diagnostics.TraceEventType.Verbose, "Calling:[usp_N_getUserScript_XML]");
						}
