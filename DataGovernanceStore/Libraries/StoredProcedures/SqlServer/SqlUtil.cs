	...
	public class SqlUtil
	{
		...
		public static DataTable ExecSql(SqlString sql, IDbConnection connection, IDbTransaction transaction)
		{
			...
			try
			{
				...
				try
				{
					using(SqlDataAdapter dataAdapter = new SqlDataAdapter("usp_execsql", (SqlConnection)connection))
					{
						...
						if (Constants.TRACING_ENABLED)
						{
							MDMTraceHelper.EmitTraceEvent(System.Diagnostics.TraceEventType.Verbose, "Calling:[usp_execsql]");
						}
		...
		public static DataTable EstimateChanges(SqlString ChangeProgram, SqlString CNodeList, IDbConnection connection, IDbTransaction transaction)
		{
			...
			try
			{
				...
				try
				{
					using(SqlDataAdapter dataAdapter = new SqlDataAdapter("Usp_Tool_EstimateChangeImpact", (SqlConnection)connection))
					{
						SqlCommand command = dataAdapter.SelectCommand;
						command.Transaction = (SqlTransaction)transaction;
						command.CommandType = CommandType.StoredProcedure;
						...
						if (Constants.TRACING_ENABLED)
						{
							MDMTraceHelper.EmitTraceEvent(System.Diagnostics.TraceEventType.Verbose, "Adding Parameter: name[@ChangeProgram]");
						}
						parameter = new SqlParameter("@ChangeProgram", System.Data.SqlDbType.VarChar, 300);
						parameter.Direction = ParameterDirection.Input;
						command.Parameters.Add(parameter);

						if (!ChangeProgram.IsNull)
						{
							if (Constants.TRACING_ENABLED)
							{
								MDMTraceHelper.EmitTraceEvent(System.Diagnostics.TraceEventType.Verbose, "Setting Parameter: name[@ChangeProgram] value[" + ChangeProgram.ToString() + "]");
							}
							parameter.Value = ChangeProgram.Value;
						}
						if (Constants.TRACING_ENABLED)
						{
						...
						if (Constants.TRACING_ENABLED)
						{
							MDMTraceHelper.EmitTraceEvent(System.Diagnostics.TraceEventType.Verbose, "Calling:[Usp_Tool_EstimateChangeImpact]");
						}
